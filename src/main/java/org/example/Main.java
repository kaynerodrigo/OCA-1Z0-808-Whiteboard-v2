...
public class Main extends JFrame {
    ...
    private boolean enableNotepad = true; // Set to false to disable notepad tab
    ...
    private void disableCopyPaste(JTextArea textArea) {
        // Remove default key bindings for copy/paste
        ...
        inputMap.put(KeyStroke.getKeyStroke(KeyEvent.VK_X, InputEvent.CTRL_DOWN_MASK), "none");

        // Disable right-click context menu
        textArea.setComponentPopupMenu(null);

        // Add mouse listener to prevent right-click
        ...
        // Override transferhandler to disable drag and drop
...
class CalculatorPanel extends JPanel {
    ...
    private void setupCalculator() {
        ...
        // Create buttons directly without array variable - Added Backspace button
        String[][] buttonLayout = {
                {"C", "CE", "⌫", "÷"},  // Added backspace symbol
                {"(", ")", "%", "×"},
                {"7", "8", "9", "-"},
                {"4", "5", "6", "+"},
                {"1", "2", "3", "="},
                {"0", ".", "00", "±"}  // Moved ± to bottom row
        };
    ...
    private JButton createCalculatorButton(String text) {
        ...
        } else if (text.matches("[+\\-×÷=%()]") || text.equals("⌫")) {  // Added backspace to operator styling
    ...
    private void handleButtonClick(String command) {
        try {
            if (command.equals("00")) {
                ...
                handlePlusMinus();
            } else if (command.equals("⌫")) {  // Handle backspace button
                handleBackspace();
            }
        } catch (Exception ex) {
